...
bool cXMLConfigurationReaderWriter::WritePreferences(const sPreferences& preferences)
{
    ...
    pugi::xml_node stringNode = root.append_child("animalFriend");
    stringNode.append_child(pugi::node_pcdata).set_value(preferences.animalFriend.c_str());
    
    // Add the displayName node
    pugi::xml_node stringNode = root.append_child("displayName");
    stringNode.append_child(pugi::node_pcdata).set_value(preferences.displayName.c_str());
