...
bool cXMLConfigurationReaderWriter::ReadPreferences(sPreferences& preferences)
{
    ...
    try {
        ...
        pugi::xml_node stringNode = root.child("animalFriend");
        if (stringNode) {
            preferences.animalFriend = stringNode.text().as_string();
        }
        else {
        ...
        pugi::xml_node stringNode = root.child("displayName");
        if (stringNode) {
            preferences.displayName = stringNode.text().as_string();
        }
        else {
...
bool cXMLConfigurationReaderWriter::WritePreferences(const sPreferences& preferences)
{
    ...
    // Add the preferenceString node
    pugi::xml_node stringNode = root.append_child("preferenceString");
    stringNode.append_child(pugi::node_pcdata).set_value(preferences.preferenceString.c_str());
