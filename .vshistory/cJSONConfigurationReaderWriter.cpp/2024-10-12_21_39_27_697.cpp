...
bool cJSONConfigurationReaderWriter::ReadLanguage(sLanguage& language, const std::string& languageFile)
{
    ...
    // Extract translations from the JSON object
    if (jsonData.contains("languageTranslations")) {
        for (const auto& item : jsonData["languageTranslations"].items()) {
            const std::string& key = item.key();               // Get the translation key
            const std::string& value = item.value();           // Get the translation value
            language.translations[key] = value;                // Insert into the map
        }
    }
    else {
        std::cerr << "No translation data found in the JSON file." << std::endl;
        return false;
    }
