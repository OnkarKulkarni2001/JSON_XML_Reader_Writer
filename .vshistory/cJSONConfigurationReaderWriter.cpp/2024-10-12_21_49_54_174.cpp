...
bool cJSONConfigurationReaderWriter::ReadLanguage(sLanguage& language, const std::string& languageFile)
{
    ...
    // Now you can access your translations safely
    if (jsonData.contains("languageTranslations")) {
        for (auto& [key, value] : jsonData["languageTranslations"].items()) {
            if (value.is_string()) {
                sLanguage languageEntry;
                languageEntry.translations[key] = value.get<std::string>();
                languages.push_back(languageEntry);  // Assuming you have a vector to hold sLanguage entries
            }
        }
    }
