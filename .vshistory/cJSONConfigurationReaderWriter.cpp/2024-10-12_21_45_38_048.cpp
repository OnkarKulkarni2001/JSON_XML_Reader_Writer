...
bool cJSONConfigurationReaderWriter::ReadLanguage(sLanguage& language, const std::string& languageFile)
{
    try {
        ...
        // Check if the "languageTranslations" key exists
        if (jsonData.contains("languageTranslations")) {
            for (auto& item : jsonData["languageTranslations"].items()) {
                std::string key = item.key();             // Get the translation key
                std::string value = item.value();         // Get the translation value
                languageData.translations[key] = value;   // Populate the translations map
            }
        }
        else {
            std::cerr << "Error: 'languageTranslations' key not found in JSON file." << std::endl;
            return false;
        }
        ...
        std::cerr << "General error reading JSON data: " << e.what() << std::endl;
        return false;
    }
}
