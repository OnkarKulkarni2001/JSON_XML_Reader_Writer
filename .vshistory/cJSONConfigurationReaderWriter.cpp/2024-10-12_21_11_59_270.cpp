...
bool cJSONConfigurationReaderWriter::ReadConfig(sConfig& config)
{
    ...
    try {
        config.preferencesFileName = jsonData.at("preferencesFileName").get<std::string>();

        // Populate language files
        for (auto& item : jsonData.at("languageFiles").items()) {
            config.languageFiles[item.key()] = item.value().get<std::string>();
        }
