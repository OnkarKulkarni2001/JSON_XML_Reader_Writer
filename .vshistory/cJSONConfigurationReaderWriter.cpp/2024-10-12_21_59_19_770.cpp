...
bool cJSONConfigurationReaderWriter::ReadLanguage(sLanguage& language, const std::string& languageFile)
{
    ...
    try {
        ...
        for (const auto& item : jsonData.at(language.translations["languageTranslations"])) {
            std::string welcome = item.at("welcome").get<std::string>();
            std::string exit = item.at("exit").get<std::string>();

            language.translations["welcome"] = welcome;
            language.translations["exit"] = exit;
        }
    }
    catch (nlohmann::json::exception& e) {
        std::cerr << "Error accessing JSON data: " << e.what() << std::endl;
